...
env = gym.make("gym_hil/PandaPickCubeMouse-v0", render_mode="human", max_episode_steps=600, x_step_size=1.0, y_step_size=1.0, z_step_size=0.2)

# 打印 wrapper 链
current = env
depth = 0
print("Wrapper chain (from outer to inner):")
while hasattr(current, 'env'):
    print(f"  [{depth}] {type(current).__name__}")
    current = current.env
    depth += 1
print(f"  [{depth}] {type(current).__name__} (base environment)")
